...
/**
 * @param documentDomain {string}
 * @param locationHost {string}
 * @param locationHref {string}
 */
export default function(documentDomain, locationHost, locationHref, perfTiming, resources, marks, measures) {
...
data.allResourcesCalc = data.resources
	...
	.map((currR, i, arr) => {
		...
		var currRes = {
			...
			isRequestToHost : urlFragments[1] === locationHost
		};
...
data.hostRequests = data.requestsOnly
	.filter((domain) => domain.domain === locationHost).length;

data.currAndSubdomainRequests = data.requestsOnly
	.filter((domain) => domain.domain.split(".").slice(-2).join(".") === locationHost.split(".").slice(-2).join("."))
	.length;

data.crossDocDomainRequests = data.requestsOnly
	.filter((domain) => !helper.endsWith(domain.domain, documentDomain)).length;

data.hostSubdomains = data.requestsByDomain
	.filter((domain) => helper.endsWith(domain.domain, locationHost.split(".").slice(-2).join(".")) && domain.domain !== locationHost)
	.length;
